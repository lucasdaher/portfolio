...
export default function Projects({ id }: ProjectsProps) {
  ...
  const filteredProjects = projects.filter(
    (projeto) =>
      !selectedTag || projeto.tags.some((tag) => tag.tag === selectedTag)
  );
      ...
      <div className="max-w-[1440px] mx-auto w-full text-white">
        ...
        <DropdownMenu>
          ...
          >
            ...
            <DropdownMenuGroup>
              <DropdownMenuItem
                onClick={() => setSelectedTag("Projeto Acadêmico")}
              >
              ...
              <DropdownMenuItem
                onClick={() => setSelectedTag("Lógica de Programação")}
              >
                <span className="cursor-pointer hover:text-white/70">
                  Lógica de Programação
                </span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setSelectedTag("Front-End")}>
                <span className="cursor-pointer hover:text-white/70">
                  Front-End
                </span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setSelectedTag("Back-End")}>
                <span className="cursor-pointer hover:text-white/70">
                  Back-End
                </span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setSelectedTag("UX Design")}>
                <span className="cursor-pointer hover:text-white/70">
                  UX Design
                </span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setSelectedTag("Design")}>
                <span className="cursor-pointer hover:text-white/70">
                  Design
                </span>
              </DropdownMenuItem>
        ...
        <div
          className={`${
            filteredProjects.length === 0
              ? ""
              : "grid grid-cols-1 gap-16 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 md:gap-x-4 md:gap-y-8"
          } w-full max-w-none mt-12 items-start`}
        >
          {filteredProjects.length === 0 ? (
            <div className="max-w-full">
              <p
                ...
                } text-base font-sora font-normal text-left transition-all duration-400 ease-in-out`}
              >
                Nenhum projeto encontrado com as definições de filtro
                informadas.
              </p>
            </div>
          ) : (
